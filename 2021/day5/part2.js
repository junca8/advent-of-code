...
readInterface.on('line', function(line) {
    ...
    } else if (Math.abs(part1[0] - part2[0]) === Math.abs(part1[1] - part2[1]) && part1[1] < part2[1]) {
        diagonalMove.push({x: part1[0], y: part1[1], distance: part2[0] - part1[0]});
    } else if (Math.abs(part1[0] - part2[0]) === Math.abs(part1[1] - part2[1]) && part1[1] > part2[1]) {
        diagonalMove.push({x: part2[0], y: part2[1], distance: part1[0] - part2[0]});
    }
});
...
function calculate() {
    ...
    for (let g = 0; g < diagonalMove.length; g++) {
        let x = diagonalMove[g].x;
        let addNum = (diagonalMove[g].distance * -1) > 0 ? -1 : 1;
        for (let q = diagonalMove[g].y; q < diagonalMove[g].y + Math.abs(diagonalMove[g].distance) + 1; q++) {
            graphArray[q][x]++;
            x += addNum;
        }
    }
