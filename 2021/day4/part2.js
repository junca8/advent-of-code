...
function calculateBingo() {
    ...
    myFileArray.forEach((file, i) => {
        bingoSequences.forEach(sequence => {
            ...
            let length = myFileArray.length - removeIndexes.length;

            if (data1 && data2 && data3 && data4 && data5 && length > 1 && !removeIndexes.includes(i)) {
                removeIndexes.push(i);
            } else if (data1 && data2 && data3 && data4 && data5 && length === 1 && !removeIndexes.includes(i)) {
                calculateFinal(i);
                throw "DONE";
            }
        });
    ...
    removeIndexes.forEach((index, i) => {
        myFileArray.splice(index - i, 1);
    });
    calculate();
}
